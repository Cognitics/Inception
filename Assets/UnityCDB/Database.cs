...
namespace Cognitics.UnityCDB
{
    public class Database : MonoBehaviour
    {
        ...
        [HideInInspector] public Cognitics.Unity.MaterialManager TerrainMaterialManager = null;
        [HideInInspector] public Cognitics.Unity.MaterialManager ModelMaterialManager = null;
        ...
        public void Initialize()
        {
            if(TerrainMaterialManager == null)
                TerrainMaterialManager = gameObject.AddComponent<Cognitics.Unity.MaterialManager>();
            if(ModelMaterialManager == null)
                ModelMaterialManager = gameObject.AddComponent<Cognitics.Unity.MaterialManager>();

            //new Material(Shader.Find("Cognitics/ModelStandard"));
            //ModelMaterialManager.Shader
        ...
        void InitLODRanges()
        {
            LODSwitchByString["Overview"] = new LODSwitch(this);
            LODSwitchByString["Overview"].InitializeDefault((float)Projection.Scale, 23);
            LODSwitchByString["Detail"] = new LODSwitch(this);
            LODSwitchByString["Detail"].InitializeDefault((float)Projection.Scale, 23);
            LODSwitchByString["GTFeatures"] = new LODSwitch(this);
            LODSwitchByString["GTFeatures"].InitializeDefault((float)Projection.Scale, 23);
            LODSwitchByString["GSFeatures"] = new LODSwitch(this);
            LODSwitchByString["GSFeatures"].InitializeDefault((float)Projection.Scale, 23);
        ...
        private void LoadLODRange(XmlElement node)
        {
            ...
            foreach (XmlElement child in node.ChildNodes)
            {
                ...
                lodSwitch.EntryDistanceByLOD[id] = indist * (float)Projection.Scale;
                lodSwitch.ExitDistanceByLOD[id] = outdist * (float)Projection.Scale;
            }
