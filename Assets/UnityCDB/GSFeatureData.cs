...
namespace Cognitics.UnityCDB
{
    ...
    public class GSFeatureData
    {
        ...
        public void QuadTreeDataUpdate(QuadTreeNode node)       // QuadTreeDelegate
        {
            ...
            for (int i = 0; i < 1; ++i)
            {
                ...
                if (Time.frameCount % 10 == 0)
                {
                    ...
                    foreach (var go in LoadingGameObjects)
                    {
                        if (!go.GetComponent<Model>().Loaded)
                            tempList.Add(go);
                    }
                    //featureData.LoadingGameObjects = tempList;
                    LoadingGameObjects = tempList;
                }
                //if (featureData.LoadingGameObjects.Count > 2)
                //    return;
                if (LoadingGameObjects.Count > 4)
                    return;
                ...
                try
                {
                    ...
                    if (modelGameObject != null)
                    {
                        ...
                        LoadingGameObjects.Add(modelGameObject);
                    }
                }
                catch (Exception e)
                {
        ...
        GameObject GenerateModel(QuadTreeNode node, NetTopologySuite.Features.Feature feature)
        {
            ...
            var cnam = feature.Attributes.Exists("CNAM") ? feature.Attributes["CNAM"].ToString() : "";
            ...
            var rotation = feature.Attributes.Exists("AO1") ? feature.Attributes["AO1"].ToString() : "0";
            ...
            model.MaterialManager = Database.ModelMaterialManager;
